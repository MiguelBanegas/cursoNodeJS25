...
export const eliminarMedicion = async (req, res) => {
    try {
        const { id } = req.params;
        const exito = await service.eliminarMedicion(id);
        if (!exito) {
            return res.status(404).json({ error: "Medición no encontrada para eliminar" });
        }
        // Si exito es true, el documento fue eliminado.
